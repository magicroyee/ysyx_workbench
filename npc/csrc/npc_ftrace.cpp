...
void npc_ftrace(word_t addr, word_t pc, int rd, int rs1) {
    ...
    if (fcall_flag) {
        indent++;
        sprintf(ftrace_buf, "[FTRACE:%08x] %*scall %s", pc, __MIN(indent, 64), "", npc_ftrace_lookup(addr));
    }
    else if (fret_flag){
        sprintf(ftrace_buf, "[FTRACE:%08x] %*sret %s", pc, __MIN(indent, 64), "", npc_ftrace_lookup(pc));
        indent--;
    }
